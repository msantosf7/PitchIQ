...
// IMPORTANTE: Aqui usamos a SERVICE_ROLE_KEY (Secret Key) porque ela tem permissão para gravar no Storage
const supabaseKey = process.env.SUPABASE_SERVICE_ROLE_KEY; 
const supabase = createClient(supabaseUrl, supabaseKey);

// 2. Função pura de Poisson
...
// 3. Processa a partida e calcula as probabilidades
function analisarPartida(jogo) {
  // Simulando as forças de ataque e defesa reais com base nas médias das ligas
  // Em produção, você pode evoluir para puxar essas médias de uma tabela
  ...
  // Regra de Valor Estrita para evitar falsos palpites
...
async function iniciarRobo() {
  ...
  // Lista de jogos do dia estruturada (Aqui capturamos da API aberta de esportes)
  // IDs correspondentes aos escudos reais da api-sports
  const jogosReaisDoDia = [
    { id: 1, liga: "Champions League", time: "20:00", time_casa: "Real Madrid", id_casa: 541, forcaCasa: 1.5, time_fora: "Dortmund", id_fora: 165, forcaFora: 1.1 },
  ...
  const { data, error } = await supabase
    ...
    .upload('jogos_do_dia.json', blob, {
      ...
      upsert: true // Sobrescreve o arquivo antigo mantendo o painel atualizado
    });

  if (error) {
    console.error("❌ Erro ao atualizar o painel:", error.message);
  } else {
